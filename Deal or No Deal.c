...
int pick_case_index(int cases[], int cases_size) {
  ...
  while (selected < 1 || selected > 26 || cases[selected - 1] == -1) {
    printf("Pick a case: ");
    scanf("%d", &selected);
  }
...
void eliminate_cases(int cases[], int cases_size, int eliminated) {
  ...
  for (int i = 0; i < eliminated; i++) {
    selected = pick_case_index(cases, cases_size);
    lost = remove_case_and_get_amount(cases, cases_size, selected);
